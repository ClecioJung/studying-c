...
int main(void) {
    ...
    for (size_t i = 0; i < x.len; i++) {
        const double result = gregory_newton_interpolation(x, y, x.data[i]);
        if (are_close(result, y.data[i], PRECISION)) {
            printf("Interpolation at %lg results in %lg\n", x.data[i], result);
        } else {
            fprintf(stderr, "Gregory-Newton interpolation: couldn't interpolate correctly the provided values!\n");
            return EXIT_FAILURE;
        }
    }
    vector_dealloc(&x);
